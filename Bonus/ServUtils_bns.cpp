...
bool Server::isBot(std::string message)
{
	size_t pos = message.find(" ");
	std::string chatbot = message.substr(pos + 1);
	chatbot.erase(chatbot.find_last_not_of(" \t\n\r") + 1);
	pos = chatbot.find(" ");
	std::string cmd = chatbot.substr(pos + 1);
	pos = cmd.find(" ");
...
void Server::executeBot(std::string message, int i)
{
	std::vector<std::string> args = parseMessage(message);
	if(args[2] == ":!roll")
	{
